...
* `linear()` function that gives a normalized linear progress value from 0 to 1 for any given amount of beats, or with a given delay
...
* `every[n]` boolean that returns true every n beats. Returns true for the duration of 1 beat. Limited from 1 to 16.
* `every_once[n]` same as `every[n]` but now the boolean returns true for only 1 frame. Useful for changing a variable onces every n beats

You can tweak the behaviour of this library with the following functions:
* `setSurfaceTitle()` to return information on BPM, beatCount and frameRate to be shown in your surface title
* `setBPM(120)` to change the amount of beats per minute for all progress functions
...
* `setInfoPanelKey('i')` to change the hotkey to hide/show the infopanel. Useful for when you have multiple infopanels.
* `disableKeyPress()` to disable responding to keypresses. If you don't disable keypresses, then these keypresses will work:
